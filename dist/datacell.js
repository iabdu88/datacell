...
function performCleaning() {
  ...
  // ── 1b. Arabic text normalization & common typo correction ──
  const AR_NORMALIZE = [
    ...
    // Remove tatweel
    [/ـ/g, ''],
    // Remove extra diacritics (tashkeel)
    [/[\u064B-\u065F]/g, ''],
    // Common Arabic typos
    [/الى(?=\s)/g, 'إلى'],
    [/اليك/g, 'إليك'],
    ...
    [/اقل/g, 'أقل'],
    [/اهم/g, 'أهم'],
    // Normalize common English typos in mixed data
    [/recieve/gi, 'receive'],
    [/occured/gi, 'occurred'],
    [/transfered/gi, 'transferred'],
    [/refrence/gi, 'reference'],
  ];
  let spellFixed = 0;
  const textOnlyCols = headers.filter(h => {
    const vals = cleanData.map(r => String(r[h] ?? '')).filter(v => v.trim());
    const arabicCount = vals.filter(v => /[\u0600-\u06FF]/.test(v)).length;
    const numericCount = vals.filter(v => !isNaN(parseFloat(v.replace(/,/g,'')))).length;
    return arabicCount > vals.length * 0.3 && numericCount < vals.length * 0.5;
  });
  textOnlyCols.forEach(col => {
    cleanData.forEach(r => {
      if (!r[col] || typeof r[col] !== 'string') return;
      let val = r[col];
      AR_NORMALIZE.forEach(([pattern, replacement]) => {
        val = val.replace(pattern, replacement);
      });
      if (val !== r[col]) { r[col] = val; spellFixed++; }
    });
  });
  if (spellFixed > 0) addOp(`📝 ${lang==='ar'?'تم تصحيح '+spellFixed+' خلية إملائياً ونصياً':'Fixed '+spellFixed+' cells (spelling/text normalization)'}`);
